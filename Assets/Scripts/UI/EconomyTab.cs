    ...
    public void UpdateGraph(float currentTreasury)
    {
        ...
        if (currentTreasury > grid.gridSize.y)
        {
           ...
           // grid.gridSize.y = Mathf.RoundToInt(currentTreasury);
            for (int i = 0; i < line.points.ToArray().Length; i++)
            {
                if (line.points[i].y >= max) max = line.points[i].y;
                if (line.points[i].y < min) min = line.points[i].y;
            }
            float dif = max - min;
            for (int i = 0; i < line.points.ToArray().Length; i++)
            {
                line.points[i] = new Vector2(line.points[i].x, line.points[i].y - min);
            }
            grid.gridSize.y = Mathf.RoundToInt(dif);
